...
def dpi1(vals): #input polynomial array, output polynomial array
    ...
    pp24 = (b*x1*diff(X_2, x1) + x2*(diff(X_2, x2) + diff(X_4, x2)) - X_2 + (x2 + x3)*diff(X_1, x3))
...
def dpi3(vals):
    ...
    X_134 = vals[2]
    X_234 = vals[3]
    pppp1234 = b*x1*diff(X_123, x1) + b*x1*diff(X_234, x4) - b*X_123 + x2*(diff(X_123, x2) - diff(X_134, x4))  - 2*X_123 + (x2 + x3)*(diff(X_123, x3) + diff(X_124, x4)) - X_123
...
def kerdn(n, degree, map): #dpi_n and map need to agree (will get outofbounds though)
    ...
    basis = sorted(itermonomials([x1, x2, x3, x4], 0, degree), key=monomial_key('grlex', [x4, x3, x2, x1])) #ordered basis
